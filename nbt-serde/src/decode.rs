...
pub fn from_reader<'a, R, T>(src: R) -> Result<T>
where
    R: io::Read,
    T: de::Deserialize<'a>,
{
...
/// Note that only maps and structs can be decoded, because the NBT format does
/// not support bare types. Other types will return `Error::NoRootCompound`.
pub fn from_gzip<'a, R, T>(src: R) -> Result<T>
where
    R: io::Read,
    T: de::Deserialize<'a>,
{
...
/// Note that only maps and structs can be decoded, because the NBT format does
/// not support bare types. Other types will return `Error::NoRootCompound`.
pub fn from_zlib<'a, R, T>(src: R) -> Result<T>
where
    R: io::Read,
    T: de::Deserialize<'a>,
{
...
impl<'de, 'a, R: io::Read> de::Deserializer<'de> for &'a mut Decoder<R> {
    type Error = Error;
    ...
    forward_to_deserialize_any! {
        ...
        str string bytes byte_buf unit seq
        tuple_struct tuple option enum
...
impl<'de, 'a, R: io::Read + 'a> de::MapAccess<'de> for MapDecoder<'a, R> {
    type Error = Error;
...
impl<'de, 'a, R: io::Read + 'a> de::SeqAccess<'de> for SeqDecoder<'a, R> {
    type Error = Error;
...
impl<'de, 'a, 'b: 'a, R: io::Read> de::Deserializer<'de> for &'b mut InnerDecoder<'a, R> {
    type Error = Error;
    ...
    forward_to_deserialize_any! {
        ...
        str string bytes byte_buf seq map
        tuple_struct struct tuple enum
        identifier ignored_any
    }
}
